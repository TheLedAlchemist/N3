...
for seed in $(seq 0 2 $((NUM_SEEDS - 1))); do
	...
	run_script $BESSEL_STATIC_SCRIPT $nextseed "${static_output_dir}/SEED_${nextseed}" $size_influence &
	run_script $BESSEL_STANDARD_SCRIPT $nextseed "${sdandard_output_dir}/SEED_${nextseed}" $size_influence
    done
